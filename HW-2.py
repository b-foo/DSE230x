...
# 
# 1. Make RDD from the data in the file given by the file path present in `hw2-files.txt`. (hint: use [the `textFile()` method](https://spark.apache.org/docs/latest/api/python/pyspark.html#pyspark.SparkContext.textFile))
...
# Your code here
rdd = sc.textFile(file_path[0]).cache()
count = rdd.count()
...
def safe_parse(raw_json):
    ...
    try:
        ...
        return None
    if 'created_at' in json_obj.keys():
        return json_obj
    else:
        return None
...
# YOUR CODE HERE
userRdd = rdd.map(safe_parse).filter(lambda x: x != None).map(lambda x: (x['user']['id_str'], x['text'])).cache()


# ## Number of unique users
# 
# Count the number of different users in all valid tweets (hint: use [the `distinct()` method](https://spark.apache.org/docs/latest/api/python/pyspark.html#pyspark.RDD.distinct)).
# 
# *******************************************************************************
# 
# **It should print**
...
# YOUR CODE HERE
users_count = len(userRdd.countByKey())
...
# YOUR CODE HERE
counts_per_partition = userRdd.map(lambda x: (partition[x[0]] if x[0] in partition.keys() else 7, 1))                .reduceByKey(lambda x,y: x+y)                .sortByKey()                .collect()
...
# YOUR CODE HERE
tokRdd = userRdd.flatMap(lambda x: tok.tokenize(x[1])).distinct()
num_of_tokens = tokRdd.count()
...
# YOUR CODE HERE
usrtokRdd = userRdd.mapValues(lambda x: set(tok.tokenize(x)))                .reduceByKey(lambda x, y: x | y)                .flatMap(lambda x: ((x[0], w) for w in x[1]))                .cache()
        
tokRdd = usrtokRdd.map(lambda x: (x[1], 1))                .reduceByKey(lambda x, y: x + y)                .filter(lambda x: x[1] >= 100)

num_freq_tokens = tokRdd.count()
top20 = tokRdd.sortBy(lambda x: x[1], ascending=False).take(20)
...
# YOUR CODE HERE
def part(k):
    return partition[k] if k in partition.keys() else 7

tokDict = dict(tokRdd.collect())
tokWord = list(tokDict.keys())

grpRdd = usrtokRdd.filter(lambda x: x[1] in tokWord)        .map(lambda x: ((part(x[0]), x[1]), 1))        .reduceByKey(lambda x, y: x + y)        .map(lambda x: (x[0][0], (x[0][1], get_rel_popularity(x[1], tokDict[x[0][1]]))))        .groupByKey()        .mapValues(lambda x: sorted(x, key=lambda y: (-y[1], y[0]), reverse=False))
            
popular_10_in_each_group =[y[:10] for x,y in sorted(grpRdd.collect())]
